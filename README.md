...
기술·제품·대분류·산업군·카테고리는 JSON 문서 다섯 개다. 저장 위치는
환경 변수 하나로 갈린다 — 코드는 고치지 않는다.

| 환경 | 저장 위치 | 설정 |
|---|---|---|
| 개발 | `data/*.json` | 없음 |
| Vercel | Vercel Blob | `BLOB_READ_WRITE_TOKEN` |
| 사내 서버 | `data/*.json` (볼륨) | 토큰 제거 |

### Vercel 에서 관리자 쓰기 켜기

1. Vercel 대시보드 → 프로젝트 → **Storage** → **Create** → **Blob**
2. 만든 Blob 을 이 프로젝트에 **Connect**
3. `BLOB_READ_WRITE_TOKEN` 이 자동으로 주입된다
4. 재배포

연결 직후에는 Blob 이 비어 있으므로 배포에 포함된 `data/*.json` 이 그대로
보인다. 관리자에서 처음 저장하는 순간부터 Blob 에 쌓이고, 이후 수정은
재배포 없이 반영된다.

### 사내 서버로 옮길 때

`BLOB_READ_WRITE_TOKEN` 을 지우고 `data/` 를 쓰기 가능한 볼륨으로 마운트하면
파일 저장으로 돌아간다. 그전에 Blob 에 쌓인 내용을 내려받아 `data/` 에
덮어써야 한다 — Blob 주소는 `<블롭도메인>/data/technologies.json` 형태다.

### 알아 둘 것

- Blob 읽기는 5초간 캐시한다. 관리자가 저장하면 그 인스턴스는 즉시 반영되고,
  다른 인스턴스는 최대 5초 늦는다.
- Blob 이 응답하지 않으면 배포에 포함된 `data/*.json` 으로 화면을 띄우고
  30초 동안은 Blob 을 건너뛴다. 공개 화면은 멈추지 않지만, 그동안의 수정
  내용은 보이지 않는다.
- 저장은 대체 수단이 없다. Blob 이 죽으면 관리자에게 오류로 알린다.
