...
export const BRAND = {
   ...
   *
   * "확인하실 수 있습니다" 로만 끝내지 않는다. 어느 사이트에나 붙는 말이라
   * 읽고 나면 남는 것이 없다. 이 사이트가 다르게 하는 일을 함께 적는다 —
   * 측정 조건을 붙이고, 공개가 제한된 항목은 그 사실을 밝히는 것.
   *
   * "실행" 이라고는 쓰지 않는다. 그 자리에서 돌려볼 수 있는 것은 api·embed
   * 타입뿐이고 나머지는 지표와 영상이다.
   */
  intro:
    '모든 성능 수치는 측정 조건과 함께 제시하며, 공개가 제한된 항목은 그 사실을 명시합니다.',
