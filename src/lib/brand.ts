...
export const BRAND = {
   ...
   * 옆에 붙을 자리가 이미 있다.
   *
   * 대분류 이름은 관리자가 고칠 수 있고 축이 늘어날 수도 있다. 문장에 박아
   * 두면 축을 하나 추가한 날부터 첫 화면이 사실과 어긋나므로, 이름은 데이터에서
   * 받아 끼운다.
   *
   * "실행" 이라고는 쓰지 않는다. 그 자리에서 돌려볼 수 있는 것은 api·embed
   * 타입뿐이고 나머지는 지표와 영상이다.
   */
  intro: (axes: readonly string[]) =>
    axes.length > 0
      ? `인피닉 연구소가 개발한 ${axes.join('·')} 기술을 공개합니다.`
      : '인피닉 연구소가 개발한 기술을 공개합니다.',
